...
At first, the device has to act as a \gls{PKG} for itself to ensure confidentiality.
...
The \gls{PKG} extracts the secret key for the device (this will be the key belonging to the \gls{PKG}s trust domain) and uses the \gls{CEK} to do a symmetric \gls{AES} encryption on the secret key. 
...
These time-based keys can then be downloaded to the mobile on a daily basis.
...
Another solution can be an \gls{ACL} based approach equivalent to what they did in~\cite{DBLP:journals/network/ShangDMBZ14}.
