\chapter{Security in Sensor Network}\label{sensor-application}
...
The \gls{ID} of every device is a part of the Name.
...
At first, the device has to act as a key generator for itself to ensure that the receiver (i.e. the \gls{PKG}) can encrypt the extracted \gls{SK}.
The trust between the device and the real \gls{PKG} is based on the concept explained in~\autoref{rendezvous_authentication}.
The device sends an Interest appending the \gls{TMPK} and the \gls{ID} to the \gls{PKG} asking to join the \gls{PKG}s trust domain.
...
Finally the requester receives the Data, verifies the signature and decrypts the sensor data.
...
\begin{enumerate}
  \item An adversary might try to eavesdrop information.
  \item An adversary might try to send bogus commands.
  \item Devices might be physically hijacked, e.g. theft of mobile.
\end{enumerate}
\todo{write more..}
...
As explained in the sequence diagram (\autoref{fig:data_pull_ibe}) presented in the above sections, each Interest appends the requester's ID (\autoref{eq:mapping-id-name-pk}).
...
Every Interest has a timestamp attached to the Name (e.g. \path{/ndn/no/ntnu/device/name2/sensorPull/1429710873778}), i.e. milliseconds from \texttt{UTC 1970-01-01 00:00:00}, that can be used for protection against replay attack. 

\subsection{Availability}
This is a harder problem to solve.
The network is purely wireless, hence vulnerable to jamming. 
An adversary could try to send infinite Interests to a device with an invalid signature, hence the device may be overloaded with work and might run out of battery fast.
...
The \gls{PK} is the Name of the device, and all content published by the device will begin with this Name, hence it is easy to verify that the publisher is the owner of the content.
