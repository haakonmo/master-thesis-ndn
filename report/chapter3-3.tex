...
Initially the idea was to have the \textit{\gls{TMPK}} appended to the content \gls{name}. 
...
\begin{description}
	\item[Init \gls{interest}] - 
  The initialization \gls{interest} seen in~\autoref{fig:init_interest-data}
  ...
  This is very useful when using \gls{IBE} and \gls{IBS}.
	\item[Init \gls{data}] - 
  The \gls{data} response the the initialization \gls{interest} contains \gls{data} with a structure defined in~\cite[messageBuf.proto]{garseg15} and illustrated in~\autoref{fig:init_interest-data}.
  \todo{explain}
	\item[Sensor \gls{interest}] -
	As in the initialization \gls{interest} the KeyLocator field is used to define the ID\textsubscript{requester}. 
  The packet is illustrated in~\autoref{fig:sensor_interest-data}.
	\item[Sensor \gls{data}] - 
  The \gls{data} response to the Sensor \gls{interest} uses the same structure as the initialization \gls{data}. 
...
The initialization protocol described in~\autoref{init} and the \gls{data} pull protocol described in~\autoref{data_pull} is tested on the computers listed in~\autoref{tbl:target_computers}.
...
\begin{table}[h]
  ...
  \gls{data} Pull                               & 61.4 ms       & 31.0 ms       & 46.2 ms     \\ %\hline
  \end{tabular}
