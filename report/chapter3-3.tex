...
As explained in~\autoref{name}, the \gls{name} structure in \gls{NDN} is left to application developers to decide.
Using \gls{IBC} it is essential to construct the \gls{name} structure where the \gls{ID} 
The \gls{ID} of every device should therefore be concatenated with the \gls{name}.
