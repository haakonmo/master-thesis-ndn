...
\newacronym{TMSK}{TMSK}{Temporary Master Secret Key}
...
{
  name=\texttt{Name},
  description={is the name of content related to a \texttt{Data} packet in NDN}
}

\newglossaryentry{interest}
{
  name=\texttt{Interest},
  description={is the packet sent from a node to another, requesting some content with a \texttt{Name}}
}
...
{
  name=\texttt{Data},
  description={is the packet sent in response to an \texttt{Interest}}
}
...
\newglossaryentry{synchronization_group}
{
  ...
  description={is group of nodes that synchronizes with a given URL}
}