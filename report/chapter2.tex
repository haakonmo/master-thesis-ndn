...
A logical though considering that they did not know what the Internet is being used for and how big it has become.
Many protocols related to Internet have been designed and deployed mainly with the goal of functionality, not thinking of security.
In the years after the birth of Internet, they soon found out that Internet needed security at several layers, due to that the application requirements and transmission importance increased.
...
The \gls{IoT} trend is coming, and the \gls{IP} network is not designed for broadcast and therefore wireless connection is not as easy as it should and could be.
...
The Data can be verified by the requester that the publisher who ``owns'' the content actually is the owner, because its cryptographically signed.
...
Now the node waits and satisfies the \gls{PIT} entry when the Data arrives, explained in~\autoref{incoming-data}. 
...
Now this trust is an issue itself. 
...
The host does not provide any assurance that the content is verified by the host itself, it only assures secure channel.
...
There is done little research on \gls{IBC} in \gls{NDN}. 
In~\cite{DBLP:conf/icnp/ZhangCXWSW11} Xinwen Zhang et al. proposes a hybrid scheme with traditional \gls{PKI} and \gls{IBC}.