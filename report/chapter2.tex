...
This is because a node in an \gls{IP} network does not know \textit{what} it processes, but rather the packets' endpoints, i.e. \textit{where} it goes and \textit{where} it comes from. 
...
In the \gls{NDN} network a \gls{node} does not have an address nor a \gls{name}.
...
The \gls{interest} finds its way to the \gls{publisher} of the file, and thus the \gls{publisher} responds with a \gls{data} packet (2) named \path{/ntnu/file1} containing the file.
...
\begin{table}[h]
  ...
  1           & 8GB/8GB         & 4GB/8GB           & 50\%/100\%      \\ %\hline
  2           & 8GB/8GB         & 4GB/4GB           & 50\%/50\%     \\ %\hline
  3           & 12GB/12GB       & 4GB/8GB           & 33\%/66\%     \\ %\hline
  Publisher   & -/12GB          & -/4GB             & -/33\%        \\ %\hline
  \end{tabular}
  \caption[Bandwidth comparison of NDN and IP]{Node bandwidth allocation comparison in IP and NDN (best case). Mobile, 2\textsuperscript{nd}device and 3\textsuperscript{rd}device requests \protect\path{/ntnu/file1} (4GB). The NDN nodes allocates bandwidth 44.3\% (in) and 66.3\% (out) compared to IP nodes.}
...
Yusnani Mohd Yussoff et al. ~\cite{DBLP:journals/corr/abs-1207-6185}
