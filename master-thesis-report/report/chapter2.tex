...
Below I will present some of the security aspects around \gls{NDN} discussed in~\cite{secure-network-content}.
Trust the content.
The desired content from the intended source and unmodified in transit.

Example from IP
Trust the host
All trust comes outside the namespace, due to the global \gls{PKI}.
Namespace-based trust\gls{SDSI}
\gls{DNS} - point to a host that speaks for the name you are interested in
\gls{CDN} often operated by \gls{ASP}
Conversation with a bank, only over \gls{TLS}, trust is based on certificates
issues: tampering, eavesdropping
Not possible over \gls{IP} to retrieve content from other than the trusted host, because of malware.
Linkage between names and host, a distributed solution.
% Chances that two distinguished hash computations are equal are negligible, hence its practical to compute this locally, i.e. do not need any distribution.

I requires security to change the network trust. 
To mathematically prove that the content originates from the believed author, and that its not modified nor been exposed to unauthorized eyes.

Idea: 
Link name and content so that author can sign this. 
Anyone can retrieve any piece of data from anyone regardless of secure channel or not.
To do this we must achieve: validity, provenance and relevance
How do we achieve this idea? 

Validity:
One solution is to do this linkage cryptographically.
Hash verification, e.g. \gls{SHA1}. \todo{link hash paper}

Provenance:
Sign data with a public key pair.
Mapping between Name and Content, cryptographically signed~\cite{secure-network-content}.
This mapping is unique.
The Name can be of any form.
Verify the authenticity and correctness
This approach is good because different rules should apply for different Names and applications. 



Relevance:
The network alone does not however provide any relevance of the data.
...
Securing Building Management Systems Using Named Data Networking~\cite{DBLP:journals/network/ShangDMBZ14}tor28mai1430