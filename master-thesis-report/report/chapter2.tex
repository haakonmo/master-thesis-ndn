...
The name can have a hierarchical structure and signatures can be added after the \gls{URI}, e.g. ``/netflix/movie/1/<signature>''.
...
\begin{itemize}
  ...
  I.e. if the Name is ``/ndn/no/ntnu'' og the Exclude contains ``/item'', the returned Data does not contain ``/ndn/no/ntnu/item''.
...
It also has a MetaInfo field that is used to specify the FreshnessPeriod amongst other things. 
